...
export const getOAuthKey = async(code) => {
	...
	const accessToken = await axios.post(tokenHost, 'grant_type=authorization_code'+'&client_id='+id+'&client_secret='+secret+'&code='+code+'&redirect_uri='+'http://localhost:5173'
	).then((res) => {
        rvalue.status = 'ok'
